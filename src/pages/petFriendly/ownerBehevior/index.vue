<template>
  <t-card>
    <t-space direction="vertical" style="width: 100%">
      <t-form v-show="showSearch" ref="queryRef" :data="queryParams" layout="inline" reset-type="initial" label-width="calc(4em + 12px)">
        ...
        <t-form-item label="页面" name="page">
          <t-input v-model="queryParams.page" placeholder="请输入页面" clearable @enter="handleQuery" />
        </t-form-item>
        <t-form-item label="标题" name="title">
          <t-input v-model="queryParams.title" placeholder="请输入标题" clearable @enter="handleQuery" />
        </t-form-item>
        <t-form-item label="行为发生时间" name="behaviorTime">
          <t-date-picker
            v-model="queryParams.behaviorTime"
            allow-input
            clearable
      ...
      >
        <template #topContent>
          <t-row>
            <t-col flex="auto">
              <t-button v-hasPermi="['petFriendly:ownerBehevior:add']" theme="primary" @click="handleAdd">
                <template #icon> <add-icon /></template>
                新增
              </t-button>
              <t-button
                v-hasPermi="['petFriendly:ownerBehevior:edit']"
                theme="default"
                variant="outline"
                :disabled="single"
                @click="handleUpdate()"
              ...
              <t-button
                v-hasPermi="['petFriendly:ownerBehevior:remove']"
                theme="danger"
                variant="outline"
                :disabled="multiple"
                @click="handleDelete()"
              ...
              <t-button v-hasPermi="['petFriendly:ownerBehevior:export']" theme="default" variant="outline" @click="handleExport">
        ...
        <template #operation="{ row }">
          <t-space :size="8" break-line>
            ...
            <my-link v-hasPermi="['petFriendly:ownerBehevior:edit']" @click.stop="handleUpdate(row)">
              <template #prefix-icon><edit-icon /></template>修改
            </my-link>
            <my-link v-hasPermi="['petFriendly:ownerBehevior:remove']" size="small" @click.stop="handleDelete(row)">
    ...
    <t-dialog
      ...
      @confirm="ownerBeheviorRef.submit()"
    >
      <t-loading :loading="buttonLoading" size="small">
        <t-form
          ref="ownerBeheviorRef"
          label-align="right"
          :data="form"
          :rules="rules"
        ...
        >
          ...
          <t-form-item label="页面" name="page">
            <t-input v-model="form.page" placeholder="请输入页面" clearable />
          </t-form-item>
          <t-form-item label="标题" name="title">
...
const ownerBeheviorRef = ref<FormInstanceFunctions>();
const open = ref(false);
const buttonLoading = ref(false);
const title = ref('');
const ownerBeheviorList = ref<PetOwnerBeheviorVo[]>([]);
...
const rules = ref<Record<string, Array<FormRule>>>({
  page: [{ max: 200, message: '页面不能超过200个字符' }],
  title: [{ max: 200, message: '标题不能超过200个字符' }],
...
// 提交表单对象
const form = ref<PetOwnerBeheviorVo & PetOwnerBeheviorForm>({});
// 查询对象
const queryParams = ref<PetOwnerBeheviorQuery>({
  pageNum: 1,
  pageSize: 10,
  status: undefined,
  ownerId: undefined,
...
function getList() {
  loading.value = true;
  listOwnerBehevior(queryParams.value)
    .then((response) => {
      ownerBeheviorList.value = response.rows;
...
function reset() {
  form.value = {};
  proxy.resetForm('ownerBeheviorRef');
}
...
function handleAdd() {
  reset();
  open.value = true;
  title.value = '添加宠物主人行为记录';
}

/** 详情按钮操作 */
function handleDetail(row: PetOwnerBeheviorVo) {
  reset();
  openView.value = true;
  openViewLoading.value = true;
  const beheviorId = row.beheviorId;
  getOwnerBehevior(beheviorId).then((response) => {
...
/** 修改按钮操作 */
function handleUpdate(row?: PetOwnerBeheviorVo) {
  buttonLoading.value = true;
  reset();
  open.value = true;
  title.value = '修改宠物主人行为记录';
  const beheviorId = row?.beheviorId || ids.value.at(0);
  getOwnerBehevior(beheviorId).then((response) => {
...
function submitForm({ validateResult, firstError }: SubmitContext) {
  if (validateResult === true) {
    ...
    if (form.value.beheviorId) {
      updateOwnerBehevior(form.value)
        .then(() => {
...
/** 删除按钮操作 */
function handleDelete(row?: PetOwnerBeheviorVo) {
  const beheviorIds = row?.beheviorId || ids.value;
  proxy.$modal.confirm(`是否确认删除宠物主人行为记录编号为${beheviorIds}的数据项？`, () => {
    const msgLoading = proxy.$modal.msgLoading('正在删除中...');
    return delOwnerBehevior(beheviorIds)
      .then(() => {
        ids.value = ArrayOps.fastDeleteElement(ids.value, beheviorIds);
        getList();
...
function handleExport() {
  proxy.download(
    'petFriendly/ownerBehevior/export',
    {
      ...queryParams.value,
    },
    `ownerBehevior_${new Date().getTime()}.xlsx`,
  );
}

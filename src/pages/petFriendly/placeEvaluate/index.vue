<template>
  <t-card>
    <t-space direction="vertical" style="width: 100%">
      ...
      >
        <template #topContent>
          <t-row>
            <t-col flex="auto">
              <t-button v-hasPermi="['petFriendly:placeEvaluate:add']" theme="primary" @click="handleAdd">
                <template #icon> <add-icon /></template>
                新增
              </t-button>
              <t-button
                v-hasPermi="['petFriendly:placeEvaluate:edit']"
                theme="default"
                variant="outline"
                :disabled="single"
                @click="handleUpdate()"
              ...
              <t-button
                v-hasPermi="['petFriendly:placeEvaluate:remove']"
                theme="danger"
                variant="outline"
                :disabled="multiple"
                @click="handleDelete()"
              ...
              <t-button v-hasPermi="['petFriendly:placeEvaluate:export']" theme="default" variant="outline" @click="handleExport">
        ...
        <template #operation="{ row }">
          <t-space :size="8" break-line>
            <my-link v-hasPermi="['petFriendly:placeEvaluate:query']" @click.stop="handleDetail(row)">
              <template #prefix-icon><browse-icon /></template>详情
            </my-link>
            <my-link v-hasPermi="['petFriendly:placeEvaluate:edit']" @click.stop="handleUpdate(row)">
              <template #prefix-icon><edit-icon /></template>修改
            </my-link>
            <my-link v-hasPermi="['petFriendly:placeEvaluate:remove']" size="small" @click.stop="handleDelete(row)">
    ...
    <t-dialog
      ...
      @confirm="placeEvaluateRef.submit()"
    >
      <t-loading :loading="buttonLoading" size="small">
        <t-form
          ref="placeEvaluateRef"
          label-align="right"
          :data="form"
          :rules="rules"
    ...
    >
      <my-descriptions :loading="openViewLoading">
        <t-descriptions-item label="ID">{{ form.evaluateId }}</t-descriptions-item>
        ...
        <t-descriptions-item label="实际消费金额">{{ form.consumptionAmount }}</t-descriptions-item>
        <t-descriptions-item label="评分">{{ form.rate }}</t-descriptions-item>
        <t-descriptions-item label="评语" :span="2">{{ form.comments }}</t-descriptions-item>
        <t-descriptions-item label="评价附图" :span="2">{{ form.pictures }}</t-descriptions-item>
        <t-descriptions-item label="评价时距离">{{ form.distance }}</t-descriptions-item>
...
const placeEvaluateRef = ref<FormInstanceFunctions>();
const open = ref(false);
const buttonLoading = ref(false);
const title = ref('');
const placeEvaluateList = ref<PetPlaceEvaluateVo[]>([]);
...
const columns = ref<Array<PrimaryTableCol>>([
  ...
  { title: `实际消费金额`, colKey: 'consumptionAmount', align: 'center' },
  { title: `评分`, colKey: 'rate', align: 'center' },
  { title: `评语`, colKey: 'comments', align: 'center', ellipsis: true },
  { title: `评价附图`, colKey: 'pictures', align: 'center', ellipsis: true },
  { title: `评价时距离`, colKey: 'distance', align: 'center' },
...
// 提交表单对象
const form = ref<PetPlaceEvaluateVo & PetPlaceEvaluateForm>({});
// 查询对象
const queryParams = ref<PetPlaceEvaluateQuery>({
  pageNum: 1,
  pageSize: 10,
  status: undefined,
  ownerId: undefined,
...
function getList() {
  loading.value = true;
  listPlaceEvaluate(queryParams.value)
    .then((response) => {
      placeEvaluateList.value = response.rows;
...
function reset() {
  form.value = {};
  proxy.resetForm('placeEvaluateRef');
}
...
function handleAdd() {
  reset();
  open.value = true;
  title.value = '添加宠物场所评价记录';
}

/** 详情按钮操作 */
function handleDetail(row: PetPlaceEvaluateVo) {
  reset();
  openView.value = true;
  openViewLoading.value = true;
  const evaluateId = row.evaluateId;
  getPlaceEvaluate(evaluateId).then((response) => {
...
/** 修改按钮操作 */
function handleUpdate(row?: PetPlaceEvaluateVo) {
  buttonLoading.value = true;
  reset();
  open.value = true;
  title.value = '修改宠物场所评价记录';
  const evaluateId = row?.evaluateId || ids.value.at(0);
  getPlaceEvaluate(evaluateId).then((response) => {
...
function submitForm({ validateResult, firstError }: SubmitContext) {
  if (validateResult === true) {
    ...
    if (form.value.evaluateId) {
      updatePlaceEvaluate(form.value)
        .then(() => {
...
/** 删除按钮操作 */
function handleDelete(row?: PetPlaceEvaluateVo) {
  const evaluateIds = row?.evaluateId || ids.value;
  proxy.$modal.confirm(`是否确认删除宠物场所评价记录编号为${evaluateIds}的数据项？`, () => {
    const msgLoading = proxy.$modal.msgLoading('正在删除中...');
    return delPlaceEvaluate(evaluateIds)
      .then(() => {
        ids.value = ArrayOps.fastDeleteElement(ids.value, evaluateIds);
        getList();
...
function handleExport() {
  proxy.download(
    'petFriendly/placeEvaluate/export',
    {
      ...queryParams.value,
    },
    `placeEvaluate_${new Date().getTime()}.xlsx`,
  );
}
